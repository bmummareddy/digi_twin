...
# Original sections untouched; Digital Twin helpers added above tabs; one new tab added.
...
# ---- Optional geometry deps (for Digital Twin only)
...
if run_recs:
    ...
    # NEW: expose Top-k to Digital Twin in the shape it expects
    dt = recs.rename(columns={
        "binder_%": "saturation_pct",
        "speed_mm_s": "roller_speed",
        "layer_um": "layer_um",
    }).copy()
    if "d50_um" not in dt.columns:
        dt["d50_um"] = float(d50_um)
    if "id" not in dt.columns:
        dt["id"] = [f"Opt-{i+1}" for i in range(len(dt))]
    # keep only needed columns (extras are fine, but this is tidy)
    keep = ["id","binder_type","saturation_pct","roller_speed","layer_um","d50_um"]
    for k in keep:
        if k not in dt.columns: dt[k] = np.nan
...
# ======================= Digital Twin helpers (must be defined before tabs) =======================
...
def _dt_scale(ax, fov_mm, length_um=500):
    ...
    y = pad*0.6
    ax.plot([x0,x1],[y,y], lw=3.0, color="#111111")
    ax.text((x0+x1)/2, y+0.02*fov_mm, f"{int(length_um)} µm",
            ha="center", va="bottom", fontsize=9, color="#111111")

def render_digital_twin(material:str, d50_um:float, layer_um:float):
    ...
    if top5 is None or getattr(top5, "empty", True):
        st.info("Generate recommendations first (click Recommend).")
        return
    ...
    def _panel_particles(ax):
        ax.add_patch(Rectangle((0,0), render_fov, render_fov, facecolor="white", edgecolor="#111111", linewidth=1.2))
        for (x,y), r in zip(centers, radii):
            ax.add_patch(Circle((x,y), r, facecolor="#2F6CF6", edgecolor="#1f2937", linewidth=0.25))
    ...
    def _panel_binder(ax, sat_pct, binder):
        ...
        ax.add_patch(Rectangle((0,0), render_fov, render_fov, facecolor=_binder_hex(binder), edgecolor="#111111", linewidth=1.2))
        ys,xs=np.where(vm)
        if len(xs):
            xm=xs*(render_fov/vm.shape[1]); ym=(vm.shape[0]-ys)*(render_fov/vm.shape[0])
            ax.scatter(xm, ym, s=0.32, c="#FFFFFF", alpha=0.96, linewidth=0)
        for (x,y), r in zip(centers, radii):
            ax.add_patch(Circle((x,y), r, facecolor="#2F6CF6", edgecolor="#1f2937", linewidth=0.25))
...
with tabs[2]:
    ...
    def rsa_pack(max_particles: int, D50_um: float, cv_pct: float, W_mult: int, seed: int, densify: bool):
        rng = np.random.default_rng(seed); cv = cv_pct/100.0
        if cv <= 0:
        ...
        diam_u = diam_um / float(D50_um); radii = 0.5 * np.sort(diam_u)[::-1]; W = float(W_mult)
        pts = []; MAX_ATTEMPTS = 40000; attempts = 0
        def can_place(x,y,r):
    ...
    X, Y = np.meshgrid(xx, yy); solid = np.zeros((Npx, Npx), dtype=bool)
    for (x, y, r) in pts:
        solid |= (X - x) ** 2 + (Y - y) ** 2 <= r ** 2
    void = ~solid
    rng_local = np.random.default_rng(int(seed) + 12345)
    idx_void = np.flatnonzero(void.ravel()); k = int(len(idx_void) * (binder_sat_pct / 100.0))
    ...
    with colA:
        ...
        axP.set_aspect('equal', 'box'); axP.add_patch(plt.Rectangle((0, 0), W, W, fill=False, linewidth=1.1, color='#111827'))
        for (x, y, r) in pts:
        ...
        plt.tight_layout(pad=0.1); st.pyplot(figP, clear_figure=True)
    ...
    with colB:
        ...
        img = np.zeros_like(solid, dtype=float); img[solid] = 0.60; img[binder_mask] = 0.95
        ...
        plt.tight_layout(pad=0.1); st.pyplot(figL, clear_figure=True)
...
# ---- Digital Twin (new tab; calls the helpers defined above)
with tabs[5]:
    render_digital_twin(material=material, d50_um=float(d50_um), layer_um=float(layer_um))
