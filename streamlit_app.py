# streamlit_app.py — BJAM Binder-Jet AM Recommender (+ Digital Twin tab, fast)
# - Keeps original recommender logic intact
# - Saves trials to st.session_state["top_recipes_df"] when you click Recommend
# - Adds a fast Digital Twin tab (hex packing, cached robust slicing)
...
if run_recs:
    ...
    # Save trials for the Digital Twin tab (non-invasive)
    st.session_state["top_recipes_df"] = recs.copy()

    recs = recs.copy()
    recs["binder_type"] = binder_family  # keep your original behavior
...
@st.cache_data(show_spinner=False)
def _crop_local(_polys_wkb: Tuple[bytes, ...], fov: float):
    ...
    half = fov/2.0
    xmin2, ymin2 = cx-half, cy-half
    win = box(xmin2, ymin2, xmin2+fov, ymin2+fov)
    clip = dom.intersection(win)
    if getattr(clip, "is_empty", True): return tuple(), (xmin2, ymin2), fov
    geoms = [clip] if isinstance(clip, Polygon) else [g for g in clip.geoms if isinstance(g, Polygon)]
    local = []
    for g in geoms:
        x, y = g.exterior.xy
        local.append(Polygon(np.c_[np.array(x)-xmin2, np.array(y)-ymin2]).wkb)
    return tuple(local), (xmin2, ymin2), fov
...
with tabs[5]:
    st.subheader("Digital Twin — STL slice + particle packing (fast)")
    if not (HAVE_TRIMESH and HAVE_SHAPELY):
        ...
        if mesh is None:
            ...
            with rcol:
                px = st.slider("Render resolution (px)", 300, 1200, 700, 50)
            ...
            if not polys_wkb:
                ...
                local_wkb, origin, fov = _crop_local(polys_wkb, fov=0.0)

                # Hex packing — fast and visually realistic
                phi_TPD = 0.90
                phi2D_target = float(np.clip(0.90*phi_TPD, 0.40, 0.88))
                centers, radii, phi2D = _hex_pack(
                    (hash(local_wkb), round(d50_unit,9), round(phi2D_target,4), round(fov,6), px),
                    local_wkb, d50_unit, phi2D_target, fov, max_particles=1800, jitter=0.12
                )

                solids = _raster_solids((hash(centers.tobytes()) if centers.size else 0, px, round(fov,6)), centers, radii, fov, px)
                pores = ~solids
                ...
                # Build images (fast)
                img_particles = np.ones((px, px, 3), dtype=float)
                ...
                img_layer = np.ones((px, px, 3), dtype=float); img_layer[:] = b_rgb
