...
_STL_OK = True
try:
    from stl import mesh as numpy_stl_mesh  # numpy-stl
except Exception:
    _STL_OK = False
...
def _mesh_from_stl_bytes(file_bytes: bytes):
    if _TRIMESH_OK:
        ...
        raise RuntimeError("Please add 'trimesh' (preferred) or 'numpy-stl' to requirements.")

def _plot_mesh_plotly(mesh_obj):
    if _TRIMESH_OK and isinstance(mesh_obj, trimesh.Trimesh):
        V = mesh_obj.vertices; F = mesh_obj.faces
    else:
        V = mesh_obj.vertices; F = mesh_obj.faces
    x, y, z = V[:,0], V[:,1], V[:,2]
    i, j, k = F[:,0], F[:,1], F[:,2]
    fig = go.Figure(data=[go.Mesh3d(x=x, y=y, z=z, i=i, j=j, k=k, opacity=0.55, name="STL")])
    fig.update_layout(scene=dict(aspectmode="data"), margin=dict(l=0,r=0,t=24,b=0))
    return fig

def _voxelize_mesh(mesh_obj, pitch: float):
    if _TRIMESH_OK and isinstance(mesh_obj, trimesh.Trimesh):
        vox = voxel_creation.voxelize(mesh_obj, pitch=pitch)
        return vox.matrix, vox.origin, pitch
    else:
        V = mesh_obj.vertices
...
def _default_layer_thickness_um(d50_um: float) -> float:
    return max(20.0, 4.0 * float(d50_um))

def _suggest_binder_settings(material: str, d50_um: float, target_td=0.90):
    base_green = 0.62
    densify_gain = float(np.clip(0.18 - 0.0015 * (d50_um - 20.0), 0.05, 0.18))
    ...
    sat = 0.78 if post_sinter >= target_td else 0.88
    vr = int(np.clip(220.0 - 2.0 * d50_um, 50.0, 200.0))
    if "sic" in mat_lower or "carbide" in mat_lower: tsint = 2150
    elif any(k in mat_lower for k in ["alumina","al2o3"]): tsint = 1600
    elif any(k in mat_lower for k in ["zirconia","zro2"]): tsint = 1450
    elif any(k in mat_lower for k in ["steel","inconel","nickel"]): tsint = 1250
    else: tsint = 1400
    return dict(binder=binder_default, saturation=round(sat,2),
                roller_speed_mm_s=vr, post_sinter_temp_C=tsint,
                est_green=round(base_green,3), est_post_sinter=round(post_sinter,3),
                meets_target=bool(post_sinter>=target_td))

def _layer_edges(z_coords, layer_thickness):
    zmin, zmax = float(np.min(z_coords)), float(np.max(z_coords))
    return np.arange(zmin, zmax + layer_thickness, layer_thickness)

def _layer_packing_estimator(occ_slice_2d, target_green=0.62):
    from scipy.ndimage import gaussian_filter
    occ = occ_slice_2d.astype(float)
    if occ.size == 0: return occ, 0.0
    sm = gaussian_filter(occ, sigma=1.0)
    m = sm.mean() if sm.mean() > 0 else 1e-6
    pf_map = np.clip(sm * (target_green / m), 0.45, 0.68)
    return pf_map, float(pf_map.mean())

def _example_cube(edge_mm=10.0):
    if _TRIMESH_OK:
        return trimesh.creation.box(extents=[edge_mm, edge_mm, edge_mm])
    ...
    class Simple: pass
    s = Simple(); s.vertices = V; s.faces = F
    return s
...
def render_digital_twin_tab():
    ...
    with st.sidebar:
        ...
        mat = st.text_input("Material powder", value="SiC", key="dt_material")
        d50 = st.number_input("Particle size D50 (μm)", min_value=1.0, max_value=200.0, value=35.0, step=1.0, key="dt_d50")
        layer_um = st.number_input("Layer thickness (μm)", min_value=10.0, max_value=500.0, value=_default_layer_thickness_um(35.0), step=5.0, key="dt_layer")
        pitch_um = st.number_input("Voxel pitch (μm)", min_value=10.0, max_value=500.0, value=_default_layer_thickness_um(35.0)/2.0, step=5.0, key="dt_pitch")

    colL, colR = st.columns([1.2, 1.0])
    with colL:
        st.subheader("Upload STL (or use sample cube)")
        f = st.file_uploader("STL file", type=["stl"], key="dt_stl")
        if f:
            bytes_data = f.read()
            try:
                ...
                mesh_obj = _example_cube()
        else:
            st.info("No STL uploaded. Using 10 mm cube sample.")
            mesh_obj = _example_cube()
    ...
    with colR:
        st.subheader("Process suggestions (heuristic)")
        recs = _suggest_binder_settings(mat, d50, target_td=0.90)
        c1, c2, c3 = st.columns(3)
        c1.metric("Binder", recs["binder"])
        c2.metric("Saturation", f"{int(recs['saturation']*100)} %")
        c3.metric("Roller speed", f"{recs['roller_speed_mm_s']} mm/s")
        st.caption(f"Post-sinter temp ≈ {recs['post_sinter_temp_C']} °C | Est. post-sinter: {int(recs['est_post_sinter']*100)}%")

    lt = float(layer_um) / 1000.0
    vp = float(pitch_um) / 1000.0

    st.subheader("Voxelization & layer slicing")
    try:
        with st.spinner("Voxelizing..."):
            occ, origin, pitch = _voxelize_mesh(mesh_obj, pitch=vp)
        nx, ny, nz = occ.shape
        st.write(f"Grid: {nx}×{ny}×{nz}, pitch ≈ {pitch*1000:.1f} μm")

        z_coords = origin[2] + np.arange(nz) * pitch
        edges = _layer_edges(z_coords, lt)
        z_centers = z_coords + 0.5 * pitch
        layer_idx = np.digitize(z_centers, edges) - 1
        L = int(layer_idx.max() + 1)

        from scipy.ndimage import zoom
        layer_means, layer_maps = [], []
        for lid in range(L):
            sel = np.where(layer_idx == lid)[0]
            if sel.size == 0:
                layer_means.append(0.0); layer_maps.append(np.zeros((nx, ny))); continue
            occ_layer = occ[:, :, sel].any(axis=2)
            pf_map, pf_mean = _layer_packing_estimator(occ_layer, target_green=0.62)
            scale = min(2, max(1, int(200 / max(occ_layer.shape[0], occ_layer.shape[1]))))
            if scale > 1: pf_map = zoom(pf_map, scale, order=1)
            layer_means.append(pf_mean); layer_maps.append(pf_map)

        st.subheader("Layer view")
        lid = st.slider("Layer index", 0, max(0, L-1), 0, 1, key="dt_layer_slider")
        pf_map = layer_maps[lid]
        fig2 = go.Figure(data=go.Heatmap(z=pf_map, coloraxis="coloraxis"))
        fig2.update_layout(coloraxis={"colorscale":"Viridis"}, margin=dict(l=0,r=0,t=20,b=0),
                           xaxis=dict(visible=False), yaxis=dict(visible=False))
        st.plotly_chart(fig2, use_container_width=True)
        st.write(f"Estimated green packing (layer {lid}): {layer_means[lid]*100:.1f}%")

        st.subheader("Per-layer average packing")
        ...
        densify_gain = float(np.clip(0.18 - 0.0015 * (d50 - 20.0), 0.05, 0.18))
        post_est = g_mean + densify_gain
        if post_est < 0.90:
            st.warning(f"Avg green ≈ {g_mean*100:.1f}% → Est. post-sinter ≈ {post_est*100:.1f}% (<90%). Consider ↑saturation, ↓roller speed, ↓layer thickness, or finer powder.")
        else:
            st.success(f"Avg green ≈ {g_mean*100:.1f}% → Est. post-sinter ≈ {post_est*100:.1f}% (≥90%).")

    except Exception as e:
        st.error(f"Digital Twin error: {e}")
        st.info("Tip: add 'trimesh' and 'scipy' to requirements (or 'numpy-stl' as fallback).")
