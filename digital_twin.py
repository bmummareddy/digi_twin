# -*- coding: utf-8 -*-
# digital_twin.py — STL slice → FOV → fast RSA packing → binder/void render
# IMPORTANT: No Streamlit calls at import time. Caching via functools.lru_cache.
...
@lru_cache(maxsize=8)
def _cube_mesh_bytes() -> Tuple[bytes, bytes]:
    if not HAVE_TRIMESH:
...
@lru_cache(maxsize=8)
def _mesh_from_bytes(mesh_bytes: bytes) -> Tuple[bytes, bytes]:
    """Return (verts_bytes, faces_bytes) as float64/int64 buffers for caching."""
    if not HAVE_TRIMESH:
        raise RuntimeError("trimesh not installed")
...
def _slice_at_z(verts_bytes: bytes, faces_bytes: bytes, z: float) -> List[bytes]:
    """Return list of WKB polygons (2D cross-section at z)."""
    if not (HAVE_TRIMESH and HAVE_SHAPELY):
        return []
    v = np.frombuffer(verts_bytes, dtype=np.float64).reshape(-1, 3)
    f = np.frombuffer(faces_bytes, dtype=np.int64).reshape(-1, 3)
...
def _pack(polys_wkb_list: List[bytes], diam_units: np.ndarray, phi_target: float,
    ...
    for d in diam:
        ...
        for _ in range(per_size):
            ...
            if not fit.contains(Point(x, y)) or not _free(x, y, r):
                continue
            idx = len(placed_xy)
...
def render(material: str, d50_um: float, layer_um: float):
    """
    Safe to import; only Streamlit is used inside this function.
    """
    ...
    with R:
        ...
        pack_fit = st.selectbox("FOV mode",
                                ["Centered square (auto)", "Manual square", "Fit full XY bounds (not square)"],
                                index=0)
        fov_mm = st.slider("Manual FOV (mm)", 0.5, 40.0, 8.0, 0.1, disabled=(pack_fit != "Manual square"))
        pack_scope = st.selectbox("Packing scope",
                                  ["Pack inside PART only", "Pack full SQUARE build box"], index=0)
    ...
    # recipe fields (fallback to sidebar vals)
    ...
    # mesh buffers for cached slicing
    verts_bytes, faces_bytes = (None, None)
    if use_cube:
        verts_bytes, faces_bytes = _cube_mesh_bytes()
    elif stl_file is not None:
        verts_bytes, faces_bytes = _mesh_from_bytes(stl_file.read())

    # layer selection
    if verts_bytes is not None and faces_bytes is not None:
        v = np.frombuffer(verts_bytes, dtype=np.float64).reshape(-1, 3)
        f = np.frombuffer(faces_bytes, dtype=np.int64).reshape(-1, 3)
    ...
    # preview 3D
    if show_mesh and verts_bytes is not None and faces_bytes is not None:
    ...
    if verts_bytes is not None and faces_bytes is not None and HAVE_SHAPELY:
        polys_wkb = _slice_at_z(verts_bytes, faces_bytes, float(z))
        if polys_wkb:
    ...
    with cA:
        ...
        if show_outline and dom is not None:
            try:
                outline = dom.exterior
                if outline is not None:
                    xg, yg = outline.xy
                    # shift to local window center
                    ox, oy, x2, y2 = (dom.bounds[0], dom.bounds[1], dom.bounds[2], dom.bounds[3])
    ...
    st.caption(
        f"FOV≈{max(render_w, render_h):.2f} mm · φ₂D(achieved)≈{min(phi2D,1.0):.2f} · "
        f"Porosity₂D≈{max(0.0, 1.0 - float(phi2D)):.2f}"
    )

    # comparison tiles (same particles; different binders/sats)
