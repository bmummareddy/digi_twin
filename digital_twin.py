...
def _mesh_to_arrays(mesh) -> Tuple[np.ndarray, np.ndarray, str]:
    v = mesh.vertices.copy()
    f = mesh.faces.copy()
    # stable-ish hash key for caching slice results
    h = hashlib.sha1(v[:64].tobytes() + f[:64].tobytes()).hexdigest()
    return v, f, h

@st.cache_data(show_spinner=False)
def _slice_at_z(mesh_key: str, verts: np.ndarray, faces: np.ndarray, z: float) -> List[bytes]:
...
# ---------- PSD + packing ----------
def _sample_psd_um(n: int, d50_um: float, seed: int) -> np.ndarray:
    rng = np.random.default_rng(seed)
    mu, sigma = np.log(max(d50_um, 1e-9)), 0.25
    d = np.exp(rng.normal(mu, sigma, size=n))
    return np.clip(d, 0.30 * d50_um, 3.0 * d50_um)

def _pack_particles(polys_wkb: List[bytes], diam_units: np.ndarray, phi_target: float,
    ...
    for d in diam:
        ...
        for _ in range(480):  # tight inner loop cap → faster
            trials += 1
...
def _voids_from_saturation(pore_mask: np.ndarray, sat: float, seed: int = 0) -> np.ndarray:
...
def render(material: str, d50_um: float, layer_um: float):
    ...
    if not (HAVE_TRIMESH and HAVE_SHAPELY):
        st.error("Install 'trimesh' and 'shapely' to use this tab (requirements.txt).")
        return

    # pull Top-5 recs prepared by your main app (unchanged)
    top5 = st.session_state.get("top5_recipes_df")
    if top5 is None or getattr(top5, "empty", True):
        st.info("Run Predict (Top-5) first to populate trials.")
        return
    ...
    with R:
        ...
        auto_micro = st.checkbox("Auto micro-FOV to reach φ with cap", value=True,
                                 help="Shrinks the square window so the particle cap can reach target φ₂D.")
        fov_mm = st.slider("Manual FOV (mm)", 0.2, 12.0, 1.5, 0.05, disabled=pack_full,
                           help="Used when Pack full slice is OFF or Auto micro-FOV is OFF.")
    ...
    diam_um = _sample_psd_um(n_psd, d50_r, seed=9991)
    ...
    if use_cube:
        ...
        mesh = _load_mesh_from_bytes(up.read())
    ...
    if mesh is not None:
        verts, faces, key = _mesh_to_arrays(mesh)
        ...
        polys_wkb = _slice_at_z(key, verts, faces, float(z))
        if not polys_wkb:
            st.warning("No section at this Z. Try a different layer.")
            return

        # bounding square of slice
        polys = [shp_wkb.loads(p) for p in polys_wkb]
        dom = unary_union(polys)
    ...
    # pack
    centers, radii, phi2D = _pack_particles(polys_local_wkb, diam_units, float(phi2D_target),
    ...
    with cB:
        ...
        vm = _voids_from_saturation(pores, sat=sat_pct/100.0, seed=123 + int(layer_idx))
    ...
    if picks:
        ...
        for i, rid in enumerate(picks[:len(cols)]):
            ...
            hexc = binder_hex(str(row.get("binder_type", "water_based")))
            pores = ~_bitmap_mask(centers, radii, render_fov, px=800)
            vm = _voids_from_saturation(pores, sat=sat/100.0, seed=987 + int(layer_idx) + i)
