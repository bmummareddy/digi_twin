# -*- coding: utf-8 -*-
# digital_twin.py — STL slice → FOV → RSA packing → binder/void render
# NOTE: This file NEVER imports streamlit. Your app passes `st` into render().
...
# ------------------------------ Mesh helpers ---------------------------------
@lru_cache(maxsize=8)
def _cube_mesh_buffers() -> Tuple[bytes, bytes]:
    if not HAVE_TRIMESH: raise RuntimeError("trimesh not installed")
    m = trimesh.creation.box(extents=(10.0, 10.0, 10.0))
    return (m.vertices.astype(np.float64).tobytes(),
            m.faces.astype(np.int64).tobytes())

@lru_cache(maxsize=8)
def _mesh_to_buffers(mesh_bytes: bytes) -> Tuple[bytes, bytes]:
    if not HAVE_TRIMESH: raise RuntimeError("trimesh not installed")
    m = trimesh.load(io.BytesIO(mesh_bytes), file_type="stl", force="mesh", process=False)
...
def _slice_at_z(verts_b: bytes, faces_b: bytes, z: float) -> List[bytes]:
    if not (HAVE_TRIMESH and HAVE_SHAPELY): return []
    v = np.frombuffer(verts_b, dtype=np.float64).reshape(-1, 3)
    f = np.frombuffer(faces_b, dtype=np.int64).reshape(-1, 3)
    m = trimesh.Trimesh(vertices=v, faces=f, process=False)
    sec = m.section(plane_origin=(0,0,z), plane_normal=(0,0,1))
    if sec is None:
        for off in (1e-4, -1e-4, 1e-3, -1e-3):
            sec = m.section(plane_origin=(0,0,z+off), plane_normal=(0,0,1))
            if sec is not None: break
...
# ------------------------------ Packing & raster ------------------------------
def _bitmap_mask(centers: np.ndarray, radii: np.ndarray, fov: float, px: int=900) -> np.ndarray:
    if len(centers) == 0: return np.zeros((px, px), dtype=bool)
...
def _void_mask(pore_mask: np.ndarray, saturation: float, seed: int) -> np.ndarray:
    ...
    while area < target and tries < 120_000:
        ...
        if pore_mask[y, x]:
            ...
            vm[add] = True; area = int(vm.sum())
    return vm

def _pack(polys_wkb: List[bytes], diam_units: np.ndarray, phi_target: float,
          max_particles: int, max_trials: int, seed: int) -> Tuple[np.ndarray, np.ndarray, float]:
    if not (HAVE_SHAPELY and polys_wkb): return np.empty((0,2)), np.empty((0,)), 0.0
    ...
    for d in diam:
        ...
        if (target_area - area_circ) / target_area < 0.08: per_size = 120
        for _ in range(per_size):
            trials += 1
            if area_circ >= target_area or len(placed_xy) >= max_particles or trials >= max_trials:
                break
            x = rng.uniform(fx0, fx1); y = rng.uniform(fy0, fy1)
            if not fit.contains(Point(x,y)) or not _free(x,y,r): continue
            idx = len(placed_xy)
...
def _scale_bar(ax, fov_mm: float, length_um: int = 500):
    ...
    pad = 0.06 * fov_mm
    x0 = fov_mm - pad - length_mm; x1 = fov_mm - pad; y = pad * 0.6
...
def render(st, *, material: str, d50_um: float, layer_um: float):
    """Render the Digital Twin tab. Pass `st` from your Streamlit app."""
    ...
    if not (HAVE_TRIMESH and HAVE_SHAPELY):
        ...
        st.error("Missing deps: " + ", ".join(missing))
        return
    ...
    with R:
        ...
        fov_mode = st.selectbox("FOV mode",
                                ["Centered square (auto)", "Manual square", "Fit full XY bounds (not square)"], 0)
        fov_mm = st.slider("Manual FOV (mm)", 0.5, 40.0, 8.0, 0.1, disabled=(fov_mode != "Manual square"))
        pack_scope = st.selectbox("Packing scope", ["Pack inside PART only", "Pack full SQUARE build box"], 0)
    ...
    # mesh buffers
    verts_b = faces_b = None
    if use_cube:
        verts_b, faces_b = _cube_mesh_buffers()
    elif stl_file is not None:
        verts_b, faces_b = _mesh_to_buffers(stl_file.read())

    # layer controls
    if verts_b is not None and faces_b is not None:
        v = np.frombuffer(verts_b, dtype=np.float64).reshape(-1, 3)
        minz, maxz = float(v[:,2].min()), float(v[:,2].max())
        ...
        layer_idx, z = 1, 0.0
        st.info("No STL — using square fallback.")

    # 3D preview
    if show_mesh and verts_b is not None and faces_b is not None:
        import plotly.graph_objects as go
        v = np.frombuffer(verts_b, dtype=np.float64).reshape(-1, 3)
        f = np.frombuffer(faces_b, dtype=np.int64).reshape(-1, 3)
        figm = go.Figure(data=[go.Mesh3d(x=v[:,0], y=v[:,1], z=v[:,2], i=f[:,0], j=f[:,1], k=f[:,2],
                                         color="lightgray", opacity=0.55, flatshading=True)])
        figm.update_layout(scene=dict(aspectmode="data"), margin=dict(l=0,r=0,t=0,b=0), height=360)
        st.plotly_chart(figm, use_container_width=True)

    # slice → FOV polys (WKB, local)
    dom = None
    polys_local: List[bytes] = []
    render_w = render_h = 0.0

    if verts_b is not None and faces_b is not None and HAVE_SHAPELY:
        polys_wkb = _slice_at_z(verts_b, faces_b, float(z))
        if polys_wkb:
            dom = unary_union([wkb.loads(p) for p in polys_wkb])
            xmin, ymin, xmax, ymax = dom.bounds
            cx, cy = dom.centroid.x, dom.centroid.y
            if fov_mode == "Fit full XY bounds (not square)":
                win = box(xmin, ymin, xmax, ymax)
            elif fov_mode == "Manual square":
                half = float(fov_mm) / 2.0; win = box(cx - half, cy - half, cx + half, cy + half)
            else:
                side = max(xmax - xmin, ymax - ymin); half = side / 2.0
            ...
            polys_local = [_to_local(pw) for pw in [g.wkb for g in geoms]]
        else:
            side = 2.0; polys_local = [box(0,0,side,side).wkb]; render_w = render_h = side
    else:
        side = 2.0; polys_local = [box(0,0,side,side).wkb]; render_w = render_h = side
    ...
    # particle count heuristic
    est = (max(render_w, render_h) * 1000.0 / max(d50_r, 1e-6))**2 * 0.34
    cap = int(np.clip(est, 900, 5200))

    # PSD & pack
    diam_um = _psd_um(7000 if fast else 10000, d50_r, cv_pct=psd_cv, seed=9991)
    unit_scale = (1e-3 if stl_units == "mm" else 1e-6)
    diam_units = diam_um * unit_scale

    centers, radii, phi2D = _pack(
        polys_local, diam_units, float(np.clip(0.95*phi2D_target, 0.45, 0.90)),
        max_particles=cap, max_trials=(180_000 if fast else 420_000), seed=20_000 + int(layer_idx)
    )
    ...
    with cA:
        ...
        if show_outline and dom is not None:
            try:
                xg, yg = dom.exterior.xy
                ox, oy = dom.bounds[0], dom.bounds[1]
                axA.plot(np.array(xg)-ox, np.array(yg)-oy, color="#222", lw=1.0, alpha=0.35)
            except Exception:
                pass
    ...
    with cB:
        px = 900
        pores = ~_bitmap_mask(centers, radii_vis, max(render_w, render_h), px)
        vmask = _void_mask(pores, saturation=sat_pct/100.0, seed=1234 + int(layer_idx))
    ...
    st.caption(f"FOV≈{max(render_w, render_h):.2f} mm · φ₂D≈{min(phi2D,1.0):.2f} · Porosity₂D≈{max(0.0,1.0-phi2D):.2f}")
